...
| `platforms/aws/main.tf` | Uses public `aws-template` VPC, security-group, and IAM-role modules, with native FortiGate ENIs/instances and optional routes. |
| `platforms/aws/outputs.tf` | Exposes VPC, instance, ENI, route, load-balancer placeholder, and implementation-gap details. |

The AWS root deliberately uses native ENIs and EC2 instances because the shared
`ec2_instance` module does not yet support FortiGate's multi-NIC, static-primary-IP,
and source/destination-check requirements.
