...
virtual-firewall deployment and the future AWS implementation. The Azure
Terraform root is under `platforms/azure/`; `platforms/aws/` is a reserved
scaffold with no AWS implementation yet. The repository also contains shared
...
The portal categorizes `docs/azure/` Markdown files as Azure architecture/design
guides, `docs/azure/runbooks/` files as deployment runbooks, and
`docs/azure/reference/` files as operations and reference material. The
`docs/aws/` folders are reserved for future AWS documentation. Adding a Markdown file requires adding it to
...
Azure DevOps plan artifact for an approved pipeline deployment.
