...
| Declarative FortiGate/GWLB route targets and failover selection | `modules/route_manager` | Used when workload routes are enabled |
...
2. Runtime HA route-target movement, health monitoring, and automatic promotion. `route_manager` provides declarative target selection only.
