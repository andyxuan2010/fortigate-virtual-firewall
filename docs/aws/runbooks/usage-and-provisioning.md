...
The FortiGate module disables source/destination checks on every FortiGate ENI. GWLB is provisioned only when `enable_internal_load_balancer` is true; it does not create GWLB endpoints or route-table steering. Workload routes are disabled by default; enabling the static route resource requires approved route table IDs and a reachable FortiGate ENI ID. In HA mode, automatic active-node route movement is still a placeholder.
