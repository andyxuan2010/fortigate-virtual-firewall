...
Run the Terraform commands in this runbook from the Azure platform root:
`platforms/azure/`.
...
- Network access to the public `andyxuan2010/azure-template` repository so
  Terraform can download the referenced modules.
...
The workflow uses the public template repository directly and does not require
a separate module-repository checkout or token.
