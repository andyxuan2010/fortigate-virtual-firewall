...
select_modules_from_changed_files() {
  ...
  while IFS= read -r changed_file; do
    ...
    case "${changed_file}" in
      ...
      platforms/azure/*.tf | platforms/azure/terraform.tfvars | \
      platforms/azure/environments/* | \
      platforms/azure/tests/* | \
      templates/* | scripts/azure/* | scripts/common/terraform-change-summary.sh)
        run_all=true
        ;;
      examples/*)
...
usage() {
  cat <<'EOF'
Usage:
  scripts/azure/module-harness-targets.sh detect --base <ref> --head <ref> --format <github-matrix|ado-matrix|json-array|newline>
  scripts/azure/module-harness-targets.sh detect-files --input <path> --format <github-matrix|ado-matrix|json-array|newline>
  scripts/azure/module-harness-targets.sh overrides --module <name> --output <path>
EOF
}
