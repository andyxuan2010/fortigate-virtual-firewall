locals {
  ...
  route_primary_target_type = var.route_next_hop_network_interface_id != null ? "network_interface_id" : var.workload_route_target_type
  route_primary_target_id = var.route_next_hop_network_interface_id != null ? var.route_next_hop_network_interface_id : (
    var.workload_route_target_id != null ? var.workload_route_target_id : (
      local.route_primary_target_type == "network_interface_id" ? module.fortigate.network_interface_ids["a-internal"] : ""
    )
  )

  route_secondary_target_id = var.workload_route_secondary_target_id != null ? var.workload_route_secondary_target_id : (
    var.workload_route_secondary_target_type == "network_interface_id" && local.mode == "ha" ? module.fortigate.network_interface_ids["b-internal"] : ""
  )

  workload_routes = {
    for route_table_id in var.workload_route_table_ids : "workload-${replace(route_table_id, "rtb-", "")}" => {
      route_table_id         = route_table_id
      destination_cidr_block = var.route_destination_cidr_block
      target = {
        type = local.route_primary_target_type
        id   = local.route_primary_target_id
      }
      failover = var.enable_route_failover ? {
        active_target = var.workload_route_failover_active_target
        secondary_target = {
          type = var.workload_route_secondary_target_type
          id   = local.route_secondary_target_id
        }
      } : null
    }
  }
