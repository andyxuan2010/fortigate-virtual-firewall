...
output "workload_route_ids" {
  ...
  value       = try(module.fortigate_route_manager[0].route_ids, {})
}

output "workload_route_active_targets" {
  description = "Effective route-manager targets keyed by workload route."
  value       = try(module.fortigate_route_manager[0].active_targets, {})
}

output "workload_route_failover_enabled" {
  description = "Whether each workload route has a declarative secondary target."
  value       = try(module.fortigate_route_manager[0].failover_enabled, {})
}
...
output "implementation_gaps" {
  ...
  value = [
    ...
    "Route-manager failover is declarative; runtime health monitoring and automatic promotion remain FortiOS or external automation responsibilities."
  ]
}
